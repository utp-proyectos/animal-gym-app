...
import type { SessionResponse, SessionRequest } from '../types'
...
export const sessionService = {
	...
	save: (payload: SessionRequest) =>
		api
			.post<ApiResponse<SessionResponse>>('/sessions', toFormData(payload))
			.then((res) => res.data.data),

	update: (id: number, payload: SessionRequest) =>
		api
			.put<ApiResponse<SessionResponse>>(`/sessions/${id}`, toFormData(payload))
			.then((res) => res.data.data),
