...
import type { PartnerEnrolledRequest, PartnerEnrolledResponse, SessionResponse } from '../types'
import {
	useGetEnrolledPartners,
	useAddSocioToSession,
	useRemoveBookingFromSession,
...
interface ModalState {
	isOpen: boolean
	data: PartnerEnrolledResponse | null
}
...
export function SessionEnrolledModal({ isOpen, onOpenChange, session }: SessionEnrolledModalProps) {
	...
	const { mutate: removeSocio } = useRemoveBookingFromSession()
	...
	const handleConfirmDelete = () => {
		if (!session?.id || !deleteModal.data?.bookingId) return

		const { bookingId, firstName, lastName } = deleteModal.data
			...
			},
			{
				onSuccess: () => {
					toast.success('Socio dado de baja', {
						description: `El socio ${firstName} ${lastName} fue retirado de la clase.`,
					})
				},
				onError: () => {
					toast.danger('Error al dar de baja', {
						description: 'No se pudo retirar al socio. Inténtalo de nuevo.',
					})
				},
			},
		)
	...
	return (
		<>
											...
											{!isLoading &&
												...
												members.map((socio) => (
													<Table.Row key={socio.bookingId}>
														...
														<Table.Cell>
															<span className="text-default-600 font-mono text-sm">
																{socio.dni}
															</span>
														</Table.Cell>
														<Table.Cell>
															<span className="text-sm text-default-500">
																{socio.enrollmentDate}
															</span>
														</Table.Cell>
														<Table.Cell>
															<div className="flex items-center justify-end pr-2">
																<Button
																	...
																	onPress={() => setDeleteModal({ isOpen: true, data: socio })}
																>
																	<Trash2 size={16} />
																</Button>
															</div>
			...
			<DeleteModal
				...
				title={`Inscripción de ${deleteModal.data?.firstName || ''}`}
